    ...
    def forward(self, inputs):
        epsilon_in = self._f_epsilon(
            torch.normal(
                ...
                std=torch.ones([self.in_size]))).to(self.device)
        epsilon_out = self._f_epsilon(
            torch.normal(
                mean=torch.zeros([self.out_size]),
                std=torch.ones([self.out_size]))).to(self.device)
        epsilon_w = torch.matmul(
            torch.unsqueeze(epsilon_in, -1),
            other=torch.unsqueeze(epsilon_out, 0)).to(self.device)
        epsilon_b = epsilon_out

        action_activation = torch.matmul(inputs, self.w + self.sigma_w * epsilon_w) + self.b + self.sigma_b * epsilon_b
